...
def gen_initialize_phono3py_fw(atoms,
                              phono3py_settings,
                              workdir,
                              atoms_spec=None,
                              calc=None,
    ...
    if atoms_spec:
        task_list.append(PyTask({"func": fw.transfer_spec.name,
                                 "args": [atoms_spec],
                                 "inputs": [atoms_spec]}))
    task_list.append(PyTask({"func": fw.initialize_phono3py.name,
                             "args": args_phono,
                             "inputs": inputs_phono,
                             "kwargs": {"symprec": symprec}}))
    task_list.append(PyTask({"func": fw.calculate_multiple.name,
...
def gen_analyze_phono3py_fw(atoms,
                            db_name,
                            phono3py_settings,
                            atoms_spec=None,
                            db_label="phonons",
    ...
    kwargs_to_db = {"symprec": symprec,
                    ...
                    "calc_type": db_label}
    task_list = []
    task_list.append(PyTask({"func": fw.calc_phono3py_force_constants.name,
                             "args": args_fc,
                             "inputs": inputs_fc}))
    task_list.append(PyTask({"func": fw.add_phonon_to_db.name,
                             "args": args_to_db,
