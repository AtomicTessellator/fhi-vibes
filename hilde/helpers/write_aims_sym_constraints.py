...
def write_sym_ijk(
    ...
    **kwargs,
    ...
    if coord_i < 0 or coord_i > 2:
        if not strict:
            print(
                "WARNING: No coord_i value was not between 0 and 2, \
    ...
    return (
        f"\nsymmetry_frac " + f"{zero_pos[0]:.5f}"
        if (op[(0 - coord_i) % 3] == "" or not use_xyz[0])
        else op[(0 - coord_i) % 3] + ", " + f"{zero_pos[1]:.5f}"
        if (op[(1 - coord_i) % 3] == "" or not use_xyz[1])
        else op[(1 - coord_i) % 3] + ", " + f"{zero_pos[2]:.5f}"
        if (op[(2 - coord_i) % 3] == "" or not use_xyz[2])
        else op[(2 - coord_i) % 3]
    )
...
def get_coord_i_from_value(coords, val=0.0, error_tolerance=1e-10, **kwargs):
    ...
    check_coords = np.where(np.abs(coords - np.floor(coords) - val) < error_tolerance)[
        0
    ]
...
def write_sym_constraints_geo(
    ...
    **kwargs,
    ...
    phonon = Phonopy(
        unitcell.to_phonopy_atoms(wrap=False),
    ...
    for aa in range(len(scaled_positions)):
        for atom in atom_list:
            ...
            if (
                int(atom.name[:2]) == supercell.numbers[aa]
                and type_num == atom.type_num
            ):
                atom.num_in_cell += 1
                break
    ...
    for aa in range(len(scaled_positions)):
        ...
        for atom in atom_list:
            type_num = atom.get_type(coord_prims[aa], atom.distortion_sym)
            if (supercell.numbers[aa] == int(atom.name[:2])) and (
                type_num == atom.type_num
            ):
                atom.num_in_cell -= 1
                # zero_pos = np.zeros(3)
                op = [
                    atom.sym_params[ii].replace("@nn", str(atom.num_in_cell))
                    for ii in range(3)
                ]
                coord_i = atom.get_coord_i(
                    coord_prims[aa],
                    val=atom.coord_i_val,
                    error_tolerance=atom.coord_i_et,
                )
                zero_pos = [
                    float(atom.zero_position[(ii - coord_i) % 3])
                    if atom.zero_position[(ii - coord_i) % 3] != "@xx"
                    else scaled_positions[aa][ii]
                    for ii in range(3)
                ]
                zero_pos = np.array(zero_pos)
                if np.any(atom.zero_position != "@xx"):
                    inds = (
                        (np.where(atom.zero_position != "@xx")[0] + coord_i) % 3
                    ).astype(int)
                    zero_pos[inds] = np.linalg.solve(
                        supercell.get_cell().T,
                        (np.dot(zero_pos, unitcell.get_cell()) + diff).T,
                    ).T[inds]
