...
An extensive user guide including a comprehensive list of the available features, tutorials, and a reference documentation is available at [`vibes.fhi-berlin.mpg.de`](http://vibes.fhi-berlin.mpg.de/). The results in [@Knoop2020] were produced by means of the presented features.

To conclude, we like to point out that _FHI-vibes_ represents an ideal framework for implementing further advanced MD methods in the future, for example the analysis of MD simulations in terms of harmonic phonons [@Turney2009] to efficiently overcome finite size- and time-effects in _ab initio_ Green Kubo simulations of thermal conductivity [@Carbogno2016].
