...
from vibes.materials_fp.material_fingerprint import get_phonon_dos_fp
...
smatrix = np.array([[-1, 1, 1], [1, -1, 1], [1, 1, -1]])
...
for phonon, sc, scs, wd in phonon_calcs:
    ...
    fp = get_phonon_bs_fp(phonon, get_special_points(atoms), binning=False)
    fp_list.append(fp)
...
phonon_dos_fp = to_dict(get_phonon_dos_fp(phonon_calcs[0][0], binning=True))
print("Phonon DOS")
