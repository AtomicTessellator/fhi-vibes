...
from vibes.materials_fp import get_phonon_bs_fp
...
def setup_workdir(atoms, smatrix):
    ...
    workdir = Path(
        "./sc_{}{}{}_{}{}{}_{}{}{}_{:.3f}_lammps".format(*smatrix.flatten(), vol)
    ).absolute()
...
def get_bs(phonon, filename):
    ...
    return get_phonon_bs_fp(phonon, binning=False)
...
def get_thermal(phonon):
    ...
    for temperature, msd, A in zip(temperatures, msds, free_ener):
        print(
            "T = {:4d} K    MSD = {:.5f} A**2, A = {:.5f} KJ/mol".format(
                temperature, msd, A
            )
        )
...
def get_fcp(
    ...
    try:
        ...
        structures = generate_mc_rattled_structures(
            supercell, number_of_structures, rattle_std, minimum_distance, seed=seed
        )
        # set up cluster space
        cs = ClusterSpace(supercell, cutoffs)

        # compute forces
        structures = calculate_multiple(
            structures, calculator, workdir="hiphive", trajectory="hiphive.traj"
        )
        # generate displacements
        structures = get_structures_with_displacements(supercell, structures)
        ...
        opt = Optimizer(sc.get_fit_data(), fit_method=fit_method, seed=seed)
        opt.train()
...
cutoff_max = inscribed_sphere_in_box(supercell.cell) - 0.01
fcp_params = {
...
scs_calculated = calculate_multiple(
    supercells_with_disps, calc, workdir=workdir, trajectory="lammps.traj"
)
...
print(
    "The frequency error between the phonopy and hiphive band structure at "
    + f"the Gamma point is: {freq_diff:.2e}"
)
print(
    "The FC2 error between the phonopy and hiphive force constant matrix is:"
    + f"{fc2_diff:.2e}"
)
